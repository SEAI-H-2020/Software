  ...
  app.get("/users/:username/:password", async (req, res) => {
    ...
    */
    try {
      ...
      const checkUsername =
        "SELECT COUNT(*) from users WHERE username = '" + getUsername + "'";
      ...
      if (userCounter == 1) {
        const user_pw =
          "SELECT (password = '" +
          encryptedPassword +
          "') AS pwd_match FROM users WHERE username = '" +
          getUsername +
          "'";
        //console.log(user_pw);
  ...
  app.get("/insertuser/:username/:password/:email", async (req, res) => {
    ...
    */
    try {
      ...
      const checkEmail =
        "SELECT COUNT(*) FROM users WHERE email = '" + getEmail + "'";
      ...
      if (mailCounter > 0) {
        ...
        else string_result = "Email já existe na DB";
      }
      ...
      if (err.code == 23505) string_result = "Utilizador já existe na DB";
    }
