...
module.exports = function (app, pool) {
  ...
  function MPStoMPH(mpsWind) {
    ...
    //console.log(
      mpsWind + " Meters per second -> " + mphWind + " Miles per hour"
    );
    return mphWind;
  }
  ...
  app.get("/csv", async (req, res) => {
    /*
        ...
        */
    try {
      const queryres = await pool.query("SELECT * from measurements");
  ...
  app.delete("/measurements", async (req, res) => {
    ...
    try {
      const queryres = await pool.query("DELETE from measurements");
      res.json(queryres);
    } catch (err) {
      //console.log(err.message);
    }
      ...
      for (var key in req.body) {
        ...
        newMeasurement = await pool.query(
          "INSERT INTO measurements (temperature, humidity, wind, noise_level, voltage) \
                    VALUES ($1, $2, $3, $4, $5)",
          [
            measurements[key].temperature,
            measurements[key].humidity,
            measurements[key].wind,
            measurements[key].noise_level,
            measurements[key].voltage,
          ]
        );
        measure_count++;
      }
  ...
  app.get("/measurements/average", async (req, res) => {
    ...
    try {
      const query =
        "select ROUND(AVG(temperature)::numeric,2) AS temperature," +
      ...
      const newMeasurement = await pool.query(query);
      newMeasurement.rows[0].temperature = parseFloat(
        newMeasurement.rows[0].temperature
      );
      newMeasurement.rows[0].humidity = parseFloat(
        newMeasurement.rows[0].humidity
      );
      newMeasurement.rows[0].wind = parseFloat(newMeasurement.rows[0].wind);
      newMeasurement.rows[0].noise_level = parseFloat(
        newMeasurement.rows[0].noise_level
      );
      newMeasurement.rows[0].id = 1;
  ...
  app.get("/measurements/average/daynight/:start/:end", async (req, res) => {
    ...
    try {
      const newMeasurement = await pool.query(
        "SELECT ROUND(AVG(temperature)::numeric,2) as temperature_day, ROUND(AVG(humidity)::numeric,2) as humidity_day, ROUND(AVG(wind)::numeric,2) as wind_day, ROUND(AVG(noise_level )::numeric,2) as noise_level_day " +
          "from measurements " +
          "where tstamp::date between date '" +
          getStart +
          "' and date '" +
          getEnd +
          "' " +
      ...
      newMeasurement.rows[0].temperature_day = parseFloat(
        newMeasurement.rows[0].temperature_day
      );
      newMeasurement.rows[0].humidity_day = parseFloat(
        newMeasurement.rows[0].humidity_day
      );
      newMeasurement.rows[0].wind_day = parseFloat(
        newMeasurement.rows[0].wind_day
      );
      newMeasurement.rows[0].noise_level_day = parseFloat(
        newMeasurement.rows[0].noise_level_day
      );
    ...
    try {
      ...
      newMeasurement.rows[0].temperature_night = parseFloat(
        newMeasurement.rows[0].temperature_night
      );
      newMeasurement.rows[0].humidity_night = parseFloat(
        newMeasurement.rows[0].humidity_night
      );
      newMeasurement.rows[0].wind_night = parseFloat(
        newMeasurement.rows[0].wind_night
      );
      newMeasurement.rows[0].noise_level_night = parseFloat(
        newMeasurement.rows[0].noise_level_night
      );
  ...
  app.get(
    "/measurements/imperial/average/daynight/:start/:end",
    async (req, res) => {
      /*
        Swagger Documentation:
        #swagger.tags = ['Measurements']
  ...
  app.get(
    "/measurements/imperial/:sensor/average/:start/:end",
    async (req, res) => {
      /*
        Swagger Documentation:
        #swagger.tags = ['Measurements']
  ...
  app.get("/measurements/:start/:end", async (req, res) => {
    ...
    try {
      const newMeasurement = await pool.query(
        "SELECT * from measurements WHERE tstamp BETWEEN '" +
          getStart +
          "' AND '" +
          getEnd +
          "' order by tstamp ASC;"
  ...
  app.get(
    "/measurements/imperial/:sensor/min/:start/:end",
    async (req, res) => {
      /*
        Swagger Documentation:
        #swagger.tags = ['Measurements']
  ...
  app.get(
    "/measurements/imperial/:sensor/max/:start/:end",
    async (req, res) => {
      /*
        Swagger Documentation:
        #swagger.tags = ['Measurements']
